define(['dart_sdk', 'packages/http_parser/http_parser', 'packages/pedantic/pedantic'], function(dart_sdk, http_parser, pedantic) {
  ...
  src__utils.mapToQuery = function(map, opts) {
    ...
    return IterableOfString()._check(pairs[$map](core.String, dart.fn(pair => dart.str(pair[$_get](0)) + "=" + dart.str(pair[$_get](1)), ListOfStringToString())))[$join]("&");
  };
  ...
  src__utils.onDone = function(T, stream, onDone) {
    return async.Stream$(T)._check(stream.transform(T, async.StreamTransformer$(T, T).fromHandlers({handleDone: dart.fn(sink => {
        sink.close();
        onDone();
  ...
  src__base_client.BaseClient = class BaseClient extends core.Object {
    ...
    read(url, opts) {
      let headers = opts && 'headers' in opts ? opts.headers : null;
      return FutureOfString()._check(this.get(url, {headers: headers}).then(core.String, dart.fn(response => {
    ...
    readBytes(url, opts) {
      let headers = opts && 'headers' in opts ? opts.headers : null;
      return FutureOfUint8List()._check(this.get(url, {headers: headers}).then(typed_data.Uint8List, dart.fn(response => {
        this[_checkResponseSuccess](url, response);
      ...
      return async.async(src__response.Response, (function* _sendUnstreamed() {
        ...
        if (body != null) {
          if (typeof body == 'string') {
            ...
            request.bodyBytes = ListOfint()._check(body[$cast](core.int));
          } else if (core.Map.is(body)) {
            request.bodyFields = MapOfString$String()._check(body[$cast](core.String, core.String));
          } else {
    ...
    send(request) {
      return async.async(src__streamed_response.StreamedResponse, (function* send() {
        ...
        pedantic$.unawaited(xhr[$onLoad].first.then(dart.void, dart.fn(_ => {
          ...
          FutureOfNull()._check(reader[$onLoad].first.then(core.Null, dart.fn(_ => {
          ...
          FutureOfNull()._check(reader[$onError].first.then(core.Null, dart.fn(error => {
